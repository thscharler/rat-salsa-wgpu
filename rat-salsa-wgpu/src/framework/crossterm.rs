    ...
    'm: {
        match event {
            ...
            winit::event::WindowEvent::CursorMoved { position, .. } => {
                let wc = window_size.pixels.width / window_size.columns_rows.width;
                let hc = window_size.pixels.height / window_size.columns_rows.height;

                track_mouse.x = position.x as u16 / wc;
                track_mouse.y = position.y as u16 / hc;

                let ct_key_modifiers = map_modifiers(modifiers);

                if track_mouse.left_pressed {
                    Some(crossterm::event::Event::Mouse(
                        crossterm::event::MouseEvent {
                            kind: crossterm::event::MouseEventKind::Drag(
                                crossterm::event::MouseButton::Left,
                            ),
                            column: track_mouse.x,
                            row: track_mouse.y,
                            modifiers: ct_key_modifiers,
                        },
                    ))
                } else if track_mouse.right_pressed {
                    Some(crossterm::event::Event::Mouse(
                        crossterm::event::MouseEvent {
                            kind: crossterm::event::MouseEventKind::Drag(
                ...
                } else if track_mouse.middle_pressed {
                    Some(crossterm::event::Event::Mouse(
                        crossterm::event::MouseEvent {
                            kind: crossterm::event::MouseEventKind::Drag(
            ...
            winit::event::WindowEvent::MouseInput { state, button, .. } => {
                let pressed = map_mouse_state(state);
                ...
                match ct_button {
                    crossterm::event::MouseButton::Left => {
                        track_mouse.left_pressed = pressed;
                    }
                    crossterm::event::MouseButton::Right => {
                        track_mouse.right_pressed = pressed;
                    }
                    crossterm::event::MouseButton::Middle => {
                        track_mouse.middle_pressed = pressed;
                    }
                }
